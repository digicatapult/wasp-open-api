...
function check_versions_consistent () {
  ...
  local HELM_VALUES_TAG_VERSION=$(yq eval '.image.tag' ./helm/wasp-open-api/values.yaml)
  local HELM_CHART_VERSION=$(yq eval '.version' ./helm/wasp-open-api/Chart.yaml)
  local HELM_CHART_APP_VERSION=$(yq eval '.appVersion' ./helm/wasp-open-api/Chart.yaml)
